...
//int total_event_network_only = 0;
//int total_event_mixed = 0;
...
int evtchn_send_network_workload_NIC(struct domain *ld, unsigned int lport) //aghax
{
    struct evtchn *lchn, *rchn;
    struct domain *rd;
    int            rport, ret = 0;
	...
	//printk("evtchn_send_network_workload_NIC\n");
    ...
    case ECS_INTERDOMAIN:
        ...
        if ( consumer_is_xen(rchn) )
            ...
            if(rd->vcpu[rchn->notify_vcpu_id]->color != 'b') //aghax
               rd->vcpu[rchn->notify_vcpu_id]->color = 'r';
            evtchn_port_set_pending(rd, rchn->notify_vcpu_id, rchn);
        }
        break;
    case ECS_IPI:
...
long do_event_channel_op(int cmd, XEN_GUEST_HANDLE_PARAM(void) arg)
{
    ...
    case EVTCHNOP_send_network_workload: { //aghax
        struct evtchn_send_network_workload send_network_workload;
        if ( copy_from_guest(&send_network_workload, arg, 1) != 0 )
            return -EFAULT;
        rc = evtchn_send_network_workload_NIC(current->domain, send_network_workload.port);
        break;
    }
