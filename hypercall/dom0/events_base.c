...
void notify_remote_via_irq_for_network_workload(int irq) //aghax
{
	int evtchn = evtchn_from_irq(irq);

	if (VALID_EVTCHN(evtchn))
		notify_remote_via_evtchn_for_network_workload(evtchn);
}
EXPORT_SYMBOL_GPL(notify_remote_via_irq_for_network_workload);
