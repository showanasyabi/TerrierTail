...
/****************************************************************************
 * (C) 2016-2017 - Esmail Asyabi - Alireza Sanaee
 ****************************************************************************
 *
...
//rbxen
#define RBXEN_NORMAL_TSLICE_MS  10
#define RBXEN_IO_I_TSLICE_MS    1
#define RBXEN_IO_A_TSLICE_MS    1


//rbxen
#define RBXEN_PCPU_STATE_NORMAL  1
#define RBXEN_PCPU_STATE_IO_I    2
#define RBXEN_PCPU_STATE_IO_A    3
...
struct csched_pcpu {
    ...
    //rbxen
    int is_migration_allowed_rebxen;
    int state_rebxen;   // which can be Normal, IO_I or IO_A
};
...
struct csched_vcpu {
    ...
    //rbxen
    int is_red_vcpu_executed_rebxen;
    int is_blue_vcpu_executed_rebxen;
};
...
static inline void __runq_tickle(struct csched_vcpu *new)
{
    ...
    struct csched_pcpu * const spc = CSCHED_PCPU(cpu); // rbxen
        ...
        for_each_csched_balance_step( balance_step )
        {
            ...
            if ( new_idlers_empty && new->pri > cur->pri)
            {
                                        ...
                                        &idle_mask) )
                {
                    ...
                    // rbxen
                    if(spc->is_migration_allowed_rebxen)
...
init_pdata(struct csched_private *prv, struct csched_pcpu *spc, int cpu)
{
    ...
    spc->is_migration_allowed_rebxen = 1;
    spc->state_rebxen = RBXEN_PCPU_STATE_NORMAL;
...
csched_vcpu_acct(struct csched_private *prv, unsigned int cpu)
{
    ...
    else if ( _csched_cpu_pick(ops, current, 0) != cpu )
    {
        ...
        // rbxen
        if(spc->is_migration_allowed_rebxen) // fixme how about above lines
        {
...
csched_alloc_vdata(const struct scheduler *ops, struct vcpu *vc, void *dd)
{
    ...
    // rbxen
    svc-> vcpu->color = 'g';
    svc->is_red_vcpu_executed_rebxen = 0;
    svc->is_blue_vcpu_executed_rebxen= 0;
...
csched_vcpu_wake(const struct scheduler *ops, struct vcpu *vc)
{
    ...
    if ( !migrating && svc->pri == CSCHED_PRI_TS_UNDER &&
         !test_bit(CSCHED_FLAG_VCPU_PARKED, &svc->flags)&& svc->vcpu->color == 'r') //
    {
...
csched_runq_steal(int peer_cpu, int cpu, int pri, int balance_step)
{
    ...
    if ( peer_pcpu != NULL && !is_idle_vcpu(peer_vcpu)  
      && peer_pcpu->is_migration_allowed_rebxen == 1 ) // rbxen
    {
...
struct csched_vcpu * find_the_first_red_vcpu(int cpu ,int snext_pri)
{
  struct list_head *iter;
  struct csched_vcpu * next_red_vcpu = NULL;
  struct list_head *  runq = RUNQ(cpu);
      list_for_each( iter, runq )
    {
      struct csched_vcpu * iter_svc = __runq_elem(iter);
      if (iter_svc->vcpu->color == 'r' &&
         iter_svc->is_red_vcpu_executed_rebxen == 0 &&
         iter_svc->pri >= snext_pri &&
         iter_svc->pri > CSCHED_PRI_IDLE)  // fixme  I am not sure about last condition
         return iter_svc;
    }
    return next_red_vcpu;
}



struct csched_vcpu  * find_the_first_blue_vcpu(int cpu ,int snext_pri)
{
  struct list_head *iter;
  struct list_head *  runq = RUNQ(cpu);
  struct csched_vcpu * next_blue_vcpu = NULL;
   list_for_each( iter, runq )
    {
       struct csched_vcpu * iter_svc = __runq_elem(iter);
       if (iter_svc->vcpu->color == 'b' &&
           iter_svc->is_blue_vcpu_executed_rebxen == 0 &&
           iter_svc->pri >= snext_pri &&
           iter_svc->pri > CSCHED_PRI_IDLE)  // fixme  I am not sure about last condition
    ...
    const struct scheduler *ops, s_time_t now, bool_t tasklet_work_scheduled)
{
    ...
    //rbxen
    struct csched_pcpu *spc = CSCHED_PCPU(cpu);
    struct csched_vcpu *rpxen_snext = NULL;
    struct list_head *iter_vcpu, *next_vcpu;
    struct csched_vcpu *svc;
    struct csched_dom *sdom;
    struct list_head *iter;
     ...
     */
    if ( vcpu_runnable(current) && !__vcpu_on_runq(scurr))   //rbxen fix2
        __runq_insert(scurr);
    else
        BUG_ON( is_idle_vcpu(current) || list_empty(runq) ); // fixme??
    ...
    // rbxen
normal_state:
    if (spc->state_rebxen == RBXEN_PCPU_STATE_NORMAL)

    {

      //rbxen
      // for each  vcpu on the runq svc-> red_execution= 0; // all vcpu to make them ready for red eecution if they have an event
      ...
      list_for_each( iter, runq )
      {
        ...
        if(snext->pri != CSCHED_PRI_IDLE){
          // rbxen
          spc->state_rebxen= RBXEN_PCPU_STATE_IO_I;  // fixme we should check if next vcpu is the current. if so, is it necessary to go to IO_I state?
        }
        goto new_out;
    ...
    if( spc->state_rebxen == RBXEN_PCPU_STATE_IO_I)
    {
      spc->is_migration_allowed_rebxen = 0;
      rpxen_snext = find_the_first_red_vcpu(cpu, snext->pri);
      if(rpxen_snext == NULL)
      {

        spc->state_rebxen = RBXEN_PCPU_STATE_IO_A;
        //for all vcpus on the runq blue exection= 0 to make them ready for blue exection if thier color are blue;
            runq = RUNQ(cpu);
            list_for_each( iter, runq )
            {
                 struct csched_vcpu * iter_svc = __runq_elem(iter);
      ...
      {
        rpxen_snext->is_red_vcpu_executed_rebxen = 1;
        rpxen_snext->vcpu->color= 'b';
        //make all its domain vcpus blue;
        sdom = rpxen_snext->sdom; // fix
        list_for_each_safe( iter_vcpu, next_vcpu, &sdom->active_vcpu )
        {
              svc = list_entry(iter_vcpu, struct csched_vcpu, active_vcpu_elem);
              svc->vcpu->color = 'b';
        ...
        tslice= MILLISECS(RBXEN_IO_I_TSLICE_MS); // we shoud make it dynamic to allow the users to change the IO_I time slice on the fly
        snext= rpxen_snext ;
    ...
    IO_A_state:
if (spc->state_rebxen == RBXEN_PCPU_STATE_IO_A)
    {
      rpxen_snext= find_the_first_blue_vcpu(cpu, snext->pri);
      if (rpxen_snext == NULL)
      {
        spc->is_migration_allowed_rebxen = 1;
        spc->state_rebxen = RBXEN_PCPU_STATE_NORMAL;

        goto normal_state;  // wher should we go from here ???????

      }
      else
      {
        rpxen_snext->is_blue_vcpu_executed_rebxen = 1;
        rpxen_snext->vcpu->color= 'g';
        tslice= MILLISECS(RBXEN_IO_A_TSLICE_MS); // we shoud make it dynamic to allow the users to change the IO_I time slice on the fly
        snext= rpxen_snext ;
