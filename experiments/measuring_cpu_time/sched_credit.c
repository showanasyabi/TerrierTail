...
static void burn_credits(struct csched_vcpu *svc, s_time_t now)
{
    ...
    //aghax
    //printk("dom: %d, vcpu: %d, delta: %lu\n", svc->sdom->dom->domain_id, svc->vcpu->vcpu_id, delta);
    ...
    const struct scheduler *ops, s_time_t now, bool_t tasklet_work_scheduled)
{
    ...
    printk("dom: %d, delta: %lu\n", scurr->vcpu->domain->domain_id, runtime);
    ...
    {
    	//aghax
    	//printk("dom: %d, delta: %lu\n", scurr->sdom->dom->domain_id, now - scurr->on_cpu);
        __runq_insert(scurr);
    }
    else
        BUG_ON( is_idle_vcpu(current) || list_empty(runq) );
    //printk("dom: %d, delta: %lu\n", scurr->vcpu->domain->domain_id, now - scurr->on_cpu);
